...
const Members = () => {
  ...
  const renderContent = () => {
    const filteredData = userData.filter((user) => user.roles.includes(activeTab));
    ...
    return (
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => handleUserClick(item)} style={styles.card}>
  ...
  const handleCall = () => {
    if (selectedUser) {
      Alert.alert(`Calling ${selectedUser.name}`, `Phone number: ${selectedUser.phoneNumber}`);
    }
  };

  const handleChat = () => {
    if (selectedUser) {
      Alert.alert(`Chatting with ${selectedUser.name}`, `Chat feature for ${selectedUser.name}`);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'Member' && styles.activeTabText]}>Members</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={[styles.tabButton, activeTab === 'Committee' && styles.activeTab]}
          onPress={() => setActiveTab('Committee')}
        >
          <Text style={[styles.tabText, activeTab === 'Committee' && styles.activeTabText]}>Committee</Text>
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.modalContent}>
            ...
            {selectedUser && (
              <>
                ...
                <Text style={styles.modalDetails}>Phone: {selectedUser.phoneNumber}</Text>
                <Text style={styles.modalDetails}>Society: {selectedUser.society}</Text>
                <Text style={styles.modalDetails}>Block: {selectedUser.building}</Text>
                <Text style={styles.modalDetails}>Flat: {selectedUser.flatNumber}</Text>
